...
//void MotorControl::recv_set_param_data() {
    ...
    //if (len >= 8) {
        //if (canbus_.whichCAN() == CAN_MODE_CLASSIC) {
            //processSetParamPacket(data, id, cmd);
        //} else if (canbus_.whichCAN() == CAN_MODE_FD) {
            //processSetParamPacketFD(data, id, cmd);
        //}
    //} else {
        //std::cout << "[ERROR] CAN data too short." << std::endl;
    //}
//}
...
void MotorControl::recv_set_param_data() {
        ...
        if (canbus_.whichCAN() == CAN_MODE_CLASSIC) {
                ...
                uint8_t cmd = 0x11;
                //std::cout << "CANID: 0x" << std::hex << id 
                        //<< ", CMD: 0x" << static_cast<int>(cmd) 
                        //<< ", DATA: ";
                //for (int i = 0; i < len; ++i) {
                ...
                if(len >= 8){
                        ...
                        std::cout << std::dec << std::endl;
                        if (len >= 8) {
                                processSetParamPacketFD(fd_frame);
                        } else {
                                std::cout << "[ERROR] CAN FD data too short." << std::endl;
                        }
                }

}

void MotorControl::processSetParamPacket(const can_frame& frame) {
    uint16_t CANID = frame.can_id;
    //uint8_t CMD = 0x11;  
    std::array<uint8_t, 8> data;
    std::memcpy(data.data(), frame.data, 8);

    if (data[2] == 0x33 || data[2] == 0x55) {
        uint16_t slaveId = static_cast<uint16_t>(data[1] << 8 | data[0]);
        uint16_t masterid = (CANID == 0x00) ? slaveId : CANID;

        if (motors_map_.find(masterid) == motors_map_.end()) {
            if (motors_map_.find(slaveId) == motors_map_.end()) {
                return;   
            } else {
                masterid = slaveId;
            }
        ...
        motors_map_[masterid]->temp_param_dict[RID] = num;
    }
}


void MotorControl::processSetParamPacketFD(const canfd_frame& frame) {
    uint16_t CANID = frame.can_id;
    //uint8_t CMD = 0x11;  
    std::array<uint8_t, 8> data;
    std::memcpy(data.data(), frame.data, 8);  
...
double MotorControl::readMotorParam(Motor& motor, int RID) {
    ...
    for (int i = 0; i < max_retries; ++i) {
        ...
        auto it = motors_map_.find(motor.SlaveID);
        if (it != motors_map_.end()) {
            auto& temp_dict = it->second->temp_param_dict;
            auto param_it = temp_dict.find(RID);
            if (param_it != temp_dict.end()) {
